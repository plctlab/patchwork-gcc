...
retry:
  ...
  /* We did not find a unit in the cache nor in the unit list, create a new
    ...
    hold a write-lock [for obvious reasons]: By separating the read/write
    lock, we will greatly reduce the contention on the read part, while the
    write part is unlikely once the unit hits the cache. */
