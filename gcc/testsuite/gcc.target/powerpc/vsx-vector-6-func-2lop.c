...
float_tests (vector float f_src_a, vector float f_src_b,
	     vector float f_and_expected, vector float f_andc_expected,
	     vector float f_nor_expected, vector float f_or_expected,
...
double_tests (vector double d_src_a, vector double d_src_b,
	      vector double d_and_expected, vector double d_andc_expected,
	      vector double d_nor_expected, vector double d_or_expected,
	      vector double d_xor_expected)
{
  /* Some of the tests are simple enough that the compiler computes the answer
     at compile time.  Put the tests in a function so the compiler will generate
     the code needed to perform the test.  */
...
main () {
  ...
  /* Run tests.  */
  float_tests (f_src_a, f_src_b, f_and_expected, f_andc_expected,
	       f_nor_expected, f_or_expected, f_xor_expected);
  double_tests (d_src_a, d_src_b, d_and_expected, d_andc_expected,
		d_nor_expected, d_or_expected, d_xor_expected);
...
/* { dg-final { scan-assembler-times {\mxxlor\M} 2 { target le }} } */
/* { dg-final { scan-assembler-times {\mxxlor\M} 12 { target be }} } */
