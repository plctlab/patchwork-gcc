...
replace_lxv_with_lxvp (rtx_insn *insn1, rtx_insn *insn2)
{
  ...
  if (regno > REGNO (insn2_dest_exp))
    {
      set_mode_and_regno (dest_exp, GET_MODE (dest_exp),
			  REGNO (insn2_dest_exp));
      set_mode_and_regno (insn2_dest_exp, GET_MODE (insn2_dest_exp), regno);
    }

  rtx opnd = gen_rtx_REG (OOmode, REGNO (dest_exp));
  PUT_MODE (src_exp, OOmode);
  lxv = gen_movoo (opnd, src_exp);
  rtx_insn *new_insn = emit_insn_before (lxv,  insn2);
  set_block_for_insn (new_insn, BLOCK_FOR_INSN (insn2));
