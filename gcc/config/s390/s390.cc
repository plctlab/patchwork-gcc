...
/* Return TRUE if FPR arguments need to be saved onto the stack due to -mpreserve-args.  */
static inline bool
  ...
  if (s390_preserve_fpr_args_p ())
    {
      ...
      max_preserve_fpr = min_preserve_fpr + crtl->args.info.fprs - 1;
    }
  ...
  if (s390_preserve_args_p && crtl->args.info.gprs)
    {
      ...
      if (crtl->args.info.gprs >= GP_ARG_NUM_REG)
	max_preserve_gpr = min_preserve_gpr + GP_ARG_NUM_REG - 1;
      else
	max_preserve_gpr = min_preserve_gpr + crtl->args.info.gprs - 1;
    }
  ...
  if (s390_preserve_fpr_arg_p (regnum) && !cfun_fpr_save_p (regnum))
    {
      ...
      add_reg_note (insn, REG_CFA_NORESTORE, reg);
...
save_gprs (rtx base, int offset, int first, int last, rtx_insn *before = NULL)
{
  ...
  if ((first >= 6 && !global_not_special_regno_p (first))
	...
	if (GET_CODE (XVECEXP (pat, 0, i)) == SET
	 ...
	 those operations with REG_CFA_NORESTORE.  */
