...
/* RAII-enabled bit stream reader.  Bools are packed into bytes.  You
...
/* RAII-enabled bit stream writer, counterpart to bits_in.  */
...
module_state::write_define (bytes_out &sec, const cpp_macro *macro)
{
  ...
  sec.c (macro->fun_like);
  sec.c (macro->variadic);
  sec.c (macro->syshdr);
...
module_state::read_define (bytes_in &sec, cpp_reader *reader) const
{
  ...
  macro->fun_like = sec.c ();
  macro->variadic = sec.c ();
  macro->syshdr = sec.c ();
